...
class TestIllegalVeto:
    ...
    @pytest.mark.parametrize(
        "note",
        [
            "This is not market manipulation, wash trading, or front-running.",
            "No spoofing, no wash trading, and no use of non-public information.",
            "The strategy does not rely on manipulation, layering, or sybil accounts.",
            "Public program under published rules; it is neither wash trading nor spoofing.",
        ],
    )
    def test_enumerated_disclaimers_in_the_legality_note_are_allowed(self, note):
        """Cost a live cycle: an honest enumerated disclaimer vetoed a clean
        strategy, because the negation cue sits 30+ chars from the last term.

        The legality note exists to say what the strategy does NOT do, so a
        cue anywhere earlier in the sentence governs the whole list."""
        idea = _idea(bot_spec=_spec(legality_note=note))
        assert bot_edge.illegal_reason(idea) is None

    def test_a_legality_note_that_admits_it_still_vetoes(self):
        idea = _idea(
            bot_spec=_spec(legality_note="Qualifying volume is generated by wash trading between two accounts.")
        )
        reason = bot_edge.illegal_reason(idea)
        assert reason is not None
        assert "legality note" in reason
