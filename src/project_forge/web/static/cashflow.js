// Cashflow board interactions. Loaded from /cashflow.
// Mirrors money_bots.js but churns with lab='cashflow' so generation +
// cashflow (time-to-first-dollar) scoring stay scoped to the board.
// CSP-compliant: no inline scripts, no innerHTML on LLM data.
(function() {
    'use strict';

    var rootEl = document.getElementById('cashflow-root');
    ...
    function renderSuccess(idea) {
        ...
        var cash = (typeof idea.cashflow_score === 'number' ? idea.cashflow_score : 0).toFixed(2);
        meta.textContent = ' — cash ' + cash + ' · mode ';
        ...
        btn.addEventListener('click', async function() {
            ...
            try {
                ...
                var resp = await fetch('/api/churn', {
                    method: 'POST',
                    headers: headers,
                    body: JSON.stringify({ lab: 'cashflow', category: categoryFilter })
                });
