...
def _git_checked(args: list[str], cwd: str) -> str:
    """Run a git command and raise on failure instead of ignoring the rc.

    Unchecked git calls are how SI commits ended up on main: a failed
    `checkout -b` left the repo on main and the follow-up add/commit
    landed there (#87).
    """
    rc, out = _run_cmd(["git"] + args, cwd=cwd)
    if rc != 0:
        raise RuntimeError(f"git {' '.join(args)} failed: {out}")
    return out


def _git_dirty_paths(root: Path | None = None) -> set[str]:
    """Paths with uncommitted changes (staged, unstaged, or untracked)."""
    out = _git_checked(["status", "--porcelain"], cwd=str(root or _PROJECT_ROOT))
    paths: set[str] = set()
...
def _validate_path(change_path: str, root: Path) -> Path:
    ...
    if not any(normalized.startswith(prefix) for prefix in _ALLOWED_PREFIXES):
        raise ValueError(f"Path not in allowed prefixes {_ALLOWED_PREFIXES}: {change_path!r}")
    return resolved
    ...
    for change in changes:
        ...
        if action == "create":
            ...
            if search not in content:
                raise ValueError(f"Search string not found in {change['path']}: {search[:80]!r}")
...
def create_improvement_pr(
    ...
    """Create a git branch, commit ONLY changed_files, and open a PR.

    Every git step is checked and the commit is refused unless HEAD is
    the SI branch — a silently failed checkout previously sent these
    commits straight to main with unrelated dirty files swept in (#87).
    """
    branch = f"self-improve-{issue_number}"
    root = str(_PROJECT_ROOT)

    # -B creates the branch or resets a stale leftover from a previous
    # cycle; plain -b fails when the branch already exists.
    _git_checked(["checkout", "-B", branch], cwd=root)
    try:
        current = _git_checked(["rev-parse", "--abbrev-ref", "HEAD"], cwd=root).strip()
        if current != branch:
            raise RuntimeError(f"on branch {current!r}, expected {branch!r} — refusing to commit")
        ...
        _git_checked(["push", "-u", "--force-with-lease", "origin", branch], cwd=root)
        return _run_gh(
            [
                "pr",
                "create",
...
async def run_self_improve_cycle() -> dict:
    ...
    for issue in issues:
        ...
        try:
            ...
            # applying on top would sweep their WIP into the SI commit (#87).
            targets = {c["path"] for c in parsed["changes"]}
            conflicts = targets & _git_dirty_paths()
            if conflicts:
                logger.warning(
                    "Skipping #%d: target files already dirty in working tree: %s",
                    issue_num,
                    sorted(conflicts),
                )
                results.append(
                    {
                        "issue": issue_num,
                        "status": "dirty_tree",
                        "detail": f"targets already modified in working tree: {sorted(conflicts)}",
                    }
                )
                continue
            ...
            if not validation["passed"]:
                logger.warning("Validation failed for #%d: %s", issue_num, validation["detail"])
